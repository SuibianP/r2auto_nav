...
def firing_mech(p_tilt, tilt, thermal, p_servo ,SERVO_INITIAL_DUTY):
    ...
    When shooting, the pusher servo is activated for 1.5s
    ...
    p_servo : pwm object from RPi.GPIO library
        p_servo is turned on and off using the start and stop methods to drive
        the pusher which moves the balls to the flywheel
        
    SERVO_INITIAL_DUTY : float
        The pusher servo is activated with this duty cycle

    Returns
    -------
    int
        When the shooting has been executed, 1 will be returned and stored in a shooting_completed variable.
        The shooting_completed variable will be checked to see if the shooting has completed.
        If the shooting has completed, the aiming node will send a message to the navigation node to take over once more
    '''
...
def main():
    '''
    Overall flow:
    1) The nodes, devices and constants are first set up
    2) If the IR target is detected, the navigation node will be instructed to stop and the aiming node will take over
    3) Aiming commences:
        a) The turtlebot checks if the target is straight ahead. If not, it will adjust until it's centered
        b) The turtlebot will then move forward until it's appropriately spaced from the target
        c) A second round of centering will take place to enhance acuracy
    4) Shooting commences
    5) Once the shooting finishes, the navigation node will be instructed to take over once more
    '''
    
    print("main function starting")
    rclpy.init()
    
    #Setting up node and publishers to communicate with navigation node and control speed of turtlebot
    qos = QoSProfile(depth=10)
    aim_node = rclpy.create_node('aiming_algorithm') 
    vel_pub = aim_node.create_publisher(Twist, 'cmd_vel', qos)
    to_nav = aim_node.create_publisher(Bool, 'shoot', qos)
    
    #Thermal camera initialisation
    i2c = busio.I2C(board.SCL, board.SDA)
    cam = adafruit_amg88xx.AMG88XX(i2c)
    scanning_threshold = 28
    distancing_threshold = 32
    
    # Specifying pins for servos
    SERVO_PIN_TILT = 21 #tilt servo
    SERVO_PIN_SERVO = 20 #continous (pusher) servo
    ...
    #Setting the starting tilt phase
    tilt = 0
